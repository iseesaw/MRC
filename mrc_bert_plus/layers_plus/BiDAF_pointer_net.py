...
class PointerNetDecoder(object):
    ...
    def decode(self, passage_vectors, question_vectors=None, init_with_question=False):
        """
...
class PointerNetwork(object):
    """PointerNetwork: Ref Stanford pytorch code"""

    def __init__(self, hidden_size):
        super(PointerNetwork, self).__init__()
        self.hidden_size = hidden_size

    def decode(self, att, mod, mask, c_len, batch_size):
        # (batch_size, seq_len, 1)
        logits_1 = tf.layers.dense(att, 1) + tf.layers.dense(mod, 1)

        # RNNEncoder
        mod_2, _ = rnn("bi-rnn", mod, [c_len] *
                    batch_size, self.hidden_size, layer_num=1)

        # (batch_size, seq_len)
        logits_2 = tf.layers.dense(att, 1) + tf.layers.dense(mod_2, 1)

        # (batch_size, seq_len)
        log_p1 = masked_softmax(tf.reshape(
            logits_1, [batch_size, c_len]), mask)
        log_p2 = masked_softmax(tf.reshape(
            logits_2, [batch_size, c_len]), mask)
